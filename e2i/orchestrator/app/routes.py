...
# Allow placeholder mode (no real file)
ALLOW_PLACEHOLDER = (os.getenv("ORCH_ALLOW_PLACEHOLDER", "true").lower() == "true")

# Allow disabling webhook HMAC verification for local dev
DISABLE_HMAC = (os.getenv("DISABLE_HMAC", "false").lower() == "true")
...
def _run_dto(r: PipelineRun) -> RunDTO:
    return RunDTO(
        runId=r.run_id,
        pipelineKey=r.pipeline.key if r.pipeline else getattr(r, "pipeline_key", None),
...
def run_pipeline(payload: RunRequest, request: Request, db: Session = Depends(get_session)):
    ...
    # fallback: strip "e2i_" prefix if present
    ...
    # handle placeholder key -> synthesize minioKey
...
def rerun(runId: str, db: Session = Depends(get_session)):
    ...
    new_run = PipelineRun(
        pipeline_id=prev.pipeline_id,
        params=prev.params or {},
...
async def task_events(request: Request, db: Session = Depends(get_session)):
    ...
    # Only verify HMAC if not disabled
    if not DISABLE_HMAC:
